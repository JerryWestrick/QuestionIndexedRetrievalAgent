...
| Host hard-reset during plumbing test | Haswell-EP / Xeon E5 v3 onnxruntime issue (see step 2 note) | `export ORT_DISABLE_CPU_AFFINITY=1` and retry |
...
| `ImportError` involving `onnxruntime` on an older CPU | CPU lacks AVX instructions onnxruntime requires | Upgrade hardware, or wait for a future corpus built with a sentence-transformers embedding function |
...
- **Upgrading the runtime.** When you replace a corpus zip, the `qira` executable is overwritten too. If multiple zips contain different versions, the one extracted last wins. If you run corpora built with different embedding functions, make sure the runtime in use supports all of them (the latest runtime in this repo selects the embedding function based on each corpus's `corpus.md`).
